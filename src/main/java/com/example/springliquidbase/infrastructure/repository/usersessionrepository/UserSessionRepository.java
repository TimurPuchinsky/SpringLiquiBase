...
public class UserSessionRepository {
    ...
    public UserSessionRepository(DbModel db) {
        this.db = db;
    }

    DbModel db;
    JwtGenerator jwtService = new JwtGenerator();
    ...
    private UserSessionEntity getEntity(UserModel model) {
        ...
        entity.setAccess_token(jwtService.generateAccessToken(model));
        entity.setAccess_token_expired(LocalDateTime.now().plusMinutes(2));
        entity.setRefresh_token(jwtService.generateRefreshToken(model));
